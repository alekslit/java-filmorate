...
-- создаём таблицу director (с режиссёрами фильма):
CREATE TABLE IF NOT EXISTS directors(
    directors_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(255) NOT NULL
);

-- создаем таблицу film_directors (связывает фильм и его режиссёра/ов)
CREATE TABLE IF NOT EXISTS film_directors (
    film_directors_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    film_id INTEGER REFERENCES films (film_id) ON DELETE CASCADE ON UPDATE CASCADE,
    directors_id INTEGER REFERENCES directors (directors_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
-- создаём таблицу отзывов reviews (связывает отзыв, фильм и пользователя) :
...
-- создаём таблицу event_feed ("Лента событий" пользователя):
